...
class build_saint(SAINT):
    def __init__(self, 
                 categories, num_continuous, dim, depth, heads, dim_head = 16, dim_out = 1, mlp_hidden_mults = (4, 2), mlp_act = None,
                 num_special_tokens = 0, attn_dropout = 0., ff_dropout = 0., cont_embeddings = 'MLP', scalingfactor = 10,
                 attentiontype = 'col', final_mlp_style = 'common', y_dim = 2, optimizer="AdamW", learning_rate=0., weight_decay=0.):
        
        super().__init__(categories, num_continuous, dim, depth, heads, dim_head, dim_out, mlp_hidden_mults, mlp_act,
        ...
        self.model = SAINT(categories, num_continuous, dim, depth, heads, dim_head, dim_out, mlp_hidden_mults, mlp_act,
                           num_special_tokens, attn_dropout, ff_dropout, cont_embeddings, scalingfactor,
                           attentiontype, final_mlp_style, y_dim)
...
class main_saint(supmodel):
    def __init__(self, params, tasktype, num_cols=[], categories=[], input_dim=0, output_dim=0, device='cuda', data_id=None, modelname="saint"):
        
        super().__init__(tasktype, params, device, data_id, modelname)
        self.model = build_saint(categories, num_cols, input_dim, params["depth"], params["heads"], params["hidden"], 
                                 mlp_hidden_mults=(4, 2), # fixed in official codes
                                 mlp_act=params["activation"], attn_dropout=params["attn_dropout"], ff_dropout=params["ff_dropout"],
                                 cont_embeddings=params["cont_embeddings"], attentiontype=params["attentiontype"], final_mlp_style=params["final_mlp_style"], y_dim=output_dim,
