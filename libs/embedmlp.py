...
class embedmlp(torch.nn.Module):
    def __init__(self, input_dim, output_dim,
                 ...
                 optimizer, learning_rate, weight_decay, cat_cols, num_cols, categories, d_embedding):
...
class build_embedmlp(embedmlp):
    def __init__(self, 
                 input_dim, output_dim, depth, width, dropout, normalization, activation,
                 optimizer, learning_rate, weight_decay, cat_cols, num_cols, categories, d_embedding):
        
        super().__init__(input_dim, output_dim, depth, width, dropout, normalization, activation, optimizer, learning_rate, weight_decay, cat_cols, num_cols, categories, d_embedding)
        self.optimizer = optimizer
        self.learning_rate = learning_rate
        self.weight_decay = weight_decay
        self.model = embedmlp(input_dim, output_dim, depth, width, dropout, normalization, activation, optimizer, learning_rate, weight_decay, cat_cols, num_cols, categories, d_embedding)
...
class embedMLP(supmodel):
    def __init__(self, params, tasktype, num_cols=[], cat_cols=[], input_dim=0, output_dim=0, device='cuda', data_id=None, modelname="embedmlp", categories=[]):
        
        super().__init__(tasktype, params, device, data_id, modelname)
        self.model = build_embedmlp(
            ...
            params["weight_decay"], cat_cols, num_cols, categories, params["d_embedding"])
