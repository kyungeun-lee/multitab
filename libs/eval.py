...
def calculate_metric(y_true, y_pred, y_prob, tasktype, datatype):
    ...
    if tasktype == "regression":
        ...
        return {f'acc_{datatype}': calculate_accuracy(y_true, y_pred), f'auroc_{datatype}': calculate_auroc(y_true, y_prob), 
                f'f1_{datatype}': calculate_f1_score(y_true, y_pred), f'logloss_{datatype}': calculate_log_loss(y_true, y_prob)}
    elif tasktype == "multiclass":
        y_true = np.argmax(y_true, axis=1)
        if y_pred.ndim == 2:
            if y_pred.shape[1] > 1:
                y_pred = np.argmax(y_pred, axis=1)
        return {f'acc_{datatype}': calculate_accuracy(y_true, y_pred), f'auroc_{datatype}': calculate_multi_auroc(y_true, y_prob), 
                f'f1_{datatype}': calculate_f1_score(y_true, y_pred, average='weighted'), f'logloss_{datatype}': calculate_log_loss(y_true, y_prob)}
