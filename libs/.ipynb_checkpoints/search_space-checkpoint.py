...
def get_search_space(trial, modelname):
    if modelname == "catboost":
        params = {
            'max_depth': trial.suggest_int('max_depth', 3, 10), #uniform int
            'learning_rate': trial.suggest_float('learning_rate', 1e-5, 1, log=True), #log uniform
...
#             'task_type': 'GPU',
    ...
    elif modelname in ["xgboost", "xgboost2"]:
        params = {
            'max_depth': trial.suggest_int('max_depth', 3, 10), #uniform int
        ...
        params = {
            ...
            'lr_scheduler': trial.suggest_categorical('lr_scheduler', [True, False]), ##KE add
            'weight_decay': trial.suggest_float("weight_decay", 1e-6, 1e-3),
            'normalization': trial.suggest_categorical('normalization', [None, "batchnorm", "layernorm"]), ##KE add
            'activation': trial.suggest_categorical('activation', ["relu", "lrelu", "sigmoid", "tanh", "gelu"]), ##KE add
            'n_epochs': 100,
            'optimizer': trial.suggest_categorical('optimizer', ["AdamW", "Adam", "sgd"]) ##KE add
        }        
        ...
        params = {
            ...
            'ffn_dropout': trial.suggest_float('attention_dropout', 0, 0.5),
        ...
        params = {
            ...
            'kv_compression': None, ## default setup in original paper
            'kv_compression_sharing': None, ## default setup in original paper
            'd_ffn_factor': trial.suggest_float('d_ffn_factor', 2/3, 8/3), ## maybe right?
            'prenormalization': trial.suggest_categorical('prenormalization', [True, False]), ## maybe right?
            'initialization': trial.suggest_categorical('initialization', ["xavier", "kaiming"]), ## maybe right?
            'activation': trial.suggest_categorical('activation', ["reglu", "geglu", "sigmoid", "relu"]), ## maybe right?
            
            'lr_scheduler': trial.suggest_categorical('lr_scheduler', [True, False]), ##KE add
            'weight_decay': trial.suggest_float("weight_decay", 1e-6, 1e-3), ##KE add
            'n_epochs': 100,
            'optimizer': trial.suggest_categorical('optimizer', ["AdamW", "Adam", "sgd"]) ##KE add
        }
    return params